...
$IS_PHYIP = $command_hash{"IS_PHYIP"};
###################################################################################################
#
...
if($IS_VERILOG == 1){
	...
	printf $FILE "module " . $VARIATION_NAME . "_loopback (\n";
		...
		for($i; $i < $NUMBER_OF_CHANNEL; $i++){
			...
			if($IS_MAC == 1 && $IS_SMALLMAC == 0  && $IS_PCS == 1 && $IS_PMA == 1 && $IS_POWERDOWN == 0){
				...
				}elsif(($IS_GXB != 0) && ($IS_ALT_RECONFIG == 1)){
					printf $FILE "  reconfig_clk" . $SEPARATOR . $TEMP_I . ",\n";
					printf $FILE "  reconfig_busy" . $SEPARATOR . $TEMP_I . ",\n";
					printf $FILE "  reconfig_togxb" . $SEPARATOR . $TEMP_I . ",\n";
					printf $FILE "  reconfig_fromgxb" . $SEPARATOR . $TEMP_I . ",\n";
				}
	...
	###################################################################################################	
		...
		for($i; $i < $NUMBER_OF_CHANNEL; $i++){
			...
			if($IS_MAC == 1 && $IS_SMALLMAC == 0  && $IS_PCS == 1 && $IS_PMA == 1 && $IS_POWERDOWN == 0){
				if($IS_PHYIP == 1){
					printf $FILE "  output [139:0] reconfig_togxb" . $SEPARATOR . $TEMP_I . ";\n";
					printf $FILE "  input [91:0] reconfig_fromgxb" . $SEPARATOR . $TEMP_I . ";\n";
				}
				else {
					if(($IS_GXB == 2) && ($IS_ALT_RECONFIG == 1)){
						printf $FILE "  output reconfig_clk" . $SEPARATOR . $TEMP_I . ";\n";
						printf $FILE "  output reconfig_busy" . $SEPARATOR . $TEMP_I . ";\n";
	...
	if ($IS_PHYIP == 0)	{
		printf $FILE "  reg reconfig_clk_tmp;\n";
		printf $FILE "  initial\n";
		printf $FILE "     reconfig_clk_tmp <= 1'b0;\n";
		printf $FILE "  always\n";
		printf $FILE "     #20 reconfig_clk_tmp <= ~reconfig_clk_tmp;\n";
    }
	...
	if($IS_MAC == 1 && $IS_PCS == 0 && $IS_PMA == 0 && $IS_GMII == 0){
		...
		for($i; $i < $NUMBER_OF_CHANNEL; $i++){
			...
			if($IS_MAC == 1 && $IS_SMALLMAC == 0  && $IS_PCS == 1 && $IS_PMA == 1 && $IS_POWERDOWN == 0){
				if($IS_PHYIP == 1){
					printf $FILE "  assign reconfig_togxb" . $SEPARATOR . $TEMP_I . "= 140'd0;\n";	
				}
				else {
					if(($IS_GXB == 2) && ($IS_ALT_RECONFIG == 1)){
						printf $FILE "  assign reconfig_clk" . $SEPARATOR . $TEMP_I . "= reconfig_clk_tmp;\n";
						printf $FILE "  assign reconfig_busy" . $SEPARATOR . $TEMP_I . "= 1'b0;\n";
						printf $FILE "  assign reconfig_togxb" . $SEPARATOR . $TEMP_I . "= 3'b000;\n";					
					}
					if(($IS_GXB == 4) && ($IS_ALT_RECONFIG == 1)){
						printf $FILE "  assign reconfig_clk" . $SEPARATOR . $TEMP_I . "= reconfig_clk_tmp;\n";
						printf $FILE "  assign reconfig_busy" . $SEPARATOR . $TEMP_I . "= 1'b0;\n";
	...
	printf $FILE "PORT (\n";
		...
		for($i; $i < $NUMBER_OF_CHANNEL; $i++){
			...
			if($IS_MAC == 1 && $IS_SMALLMAC == 0  && $IS_PCS == 1 && $IS_PMA == 1 && $IS_POWERDOWN == 0){
				if($IS_PHYIP == 1){
					printf $FILE "   reconfig_togxb" . $SEPARATOR . $TEMP_I . " : OUT STD_LOGIC_VECTOR (139 DOWNTO 0);\n";
					printf $FILE "   reconfig_fromgxb" . $SEPARATOR . $TEMP_I . " : IN STD_LOGIC_VECTOR (91 DOWNTO 0);\n";
				}
				else {
					if(($IS_GXB == 2) && ($IS_ALT_RECONFIG == 1)){
						printf $FILE "   reconfig_clk" . $SEPARATOR . $TEMP_I . " : OUT STD_LOGIC;\n";
						printf $FILE "   reconfig_busy" . $SEPARATOR . $TEMP_I . " : OUT STD_LOGIC;\n";
	...
	if ($IS_PHYIP == 0)	{
		printf $FILE "   signal reconfig_clk_tmp :  STD_LOGIC;\n";
	}
	...
	if ($IS_PHYIP == 0)	{
		printf $FILE "   process\n";
		printf $FILE "      begin\n";
		printf $FILE "         reconfig_clk_tmp <= '0' ;\n";
		printf $FILE "         wait for 20 ns ;\n";
		printf $FILE "         reconfig_clk_tmp <= '1' ;\n";
		printf $FILE "         wait for 20 ns ;\n";
		printf $FILE "      end process ;\n";
		printf $FILE "\n";
    }
	...
	if($IS_MAC == 1 && $IS_PCS == 0 && $IS_PMA == 0 && $IS_GMII == 0){
		...
		for($i; $i < $NUMBER_OF_CHANNEL; $i++){
			...
			if($IS_MAC == 1 && $IS_SMALLMAC == 0  && $IS_PCS == 1 && $IS_PMA == 1 && $IS_POWERDOWN == 0){
				if($IS_PHYIP == 1){
					printf $FILE "   reconfig_togxb" . $SEPARATOR . $TEMP_I . " <= conv_std_logic_vector(0, 140);\n";
				}
				else {
					if(($IS_GXB == 2) && ($IS_ALT_RECONFIG == 1)){
						printf $FILE "   reconfig_clk" . $SEPARATOR . $TEMP_I . " <= reconfig_clk_tmp;\n";
						printf $FILE "   reconfig_busy" . $SEPARATOR . $TEMP_I . " <= '0';\n";
						printf $FILE "   reconfig_togxb" . $SEPARATOR . $TEMP_I . " <= \"000\";\n";
					}
					if(($IS_GXB == 4) && ($IS_ALT_RECONFIG == 1)){
						printf $FILE "   reconfig_clk" . $SEPARATOR . $TEMP_I . " <= reconfig_clk_tmp;\n";
						printf $FILE "   reconfig_busy" . $SEPARATOR . $TEMP_I . " <= '0';\n";
